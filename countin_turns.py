...
def handle_qr(qr_data):
    ...
    global playmobil_detections, cone_detections
    if command == "turn right":
        ...
        print("Executing Left Circle roundabout with rounds:", rounds)
        log_event("Left Circle roundabout executed with rounds: " + str(rounds))
        left_roundabout(rounds)
    elif command == "right circle":
        rounds = playmobil_detections + cone_detections
        print("Executing Right Circle roundabout with rounds:", rounds)
        ...
        log_event("Unknown QR code content: " + qr_data)
...
while True:
    ...
    if bottom_right < threshold and bottom_left < threshold:
        zumi.reverse(speed=8, duration=0.3)
        zumi.turn_right(75)
        ir_readings = zumi.get_all_IR_data()
        bottom_right = ir_readings[1]
        bottom_left = ir_readings[3]
        ...
        if not turn_counted and abs(current_angle) > 60:
            ...
            print("Kurve gezählt, aktuelle Anzahl:", turns)
            turn_counted = True
    elif turn_counted and abs(current_angle) < 30:
        turn_counted = False
        zumi.reset_gyro()
    elif bottom_right < threshold:
